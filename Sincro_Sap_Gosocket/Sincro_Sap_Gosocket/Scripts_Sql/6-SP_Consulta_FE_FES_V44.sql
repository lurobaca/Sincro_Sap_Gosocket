...
ALTER PROCEDURE [dbo].[SP_Consulta_FE_FES_V44]
    @DocNum VARCHAR(50),
...
BEGIN
    SET NOCOUNT ON;
      ...
      -- CTEs completas requeridas por el procedimiento
    WITH  Emisor AS (
    SELECT TOP 1
   ...
   --     RIGHT('00' + ISNULL(OADM.U_LDT_District,''), 2)  AS Emisor_Distrito,
    ...
    Receptor AS (
        SELECT
			INV.U_LDT_ActEcoRec  AS CodigoActividadReceptor,	  
 ...
 TipoFactura AS (
    SELECT
        T0.DocEntry,
        CASE
            WHEN MAX(ISNULL(C.U_LDT_TipoDocElect, '')) = '04' THEN '09' -- FEE
            WHEN MAX(ISNULL(C.U_LDT_TipoDocElect, '')) = '03' THEN '04' -- TE
    ...
    OUTER APPLY (
        SELECT
            CASE
                ...
                THEN
                    TRY_CONVERT(decimal(10,5), REPLACE(SUBSTRING(LTRIM(RTRIM(T1.TaxCode)), 4, 50), ',', '.'))
...
ImpuestosEspecificos AS (
    SELECT 
        ...
        0.0 AS DetalleServicio_ImpuestoEspecifico_Porcentaje,  -- omitido si no aplica
        0.0 AS DetalleServicio_ImpuestoEspecifico_Proporcion,  -- omitido si no aplica
...
/*==============================================================
  ...
  2) Para convertir usa el tipo de cambio digitado en SAP:
        - Primero INV1.Rate, porque corresponde al tipo de cambio de la línea.
        - Si INV1.Rate viene 0 o NULL, usa OINV.DocRate.
...
ImpuestoBase AS (
    SELECT 
        ...
        CASE
            ...
            THEN ROUND(DS.DetalleServicio_SubTotal,5)
            WHEN CI.DetalleServicio_ImpuestoCodigoTarifa IN ('02','04','05')
                THEN CASE 
                        WHEN T1.TaxOnly='Y' THEN ROUND(DS.DetalleServicio_MontoTotal,5)
...
Exoneracion AS (
    SELECT
        ...
        CASE  
            WHEN TF.CodigoTipoDocumento = '09' THEN NULL 
        ...
        CASE  
            WHEN TF.CodigoTipoDocumento = '09' THEN NULL 
            ELSE CASE 
                WHEN NULLIF(LTRIM(RTRIM(ISNULL(T1.U_LDT_TipoExo,''))), '') IS NOT NULL THEN
                    ROUND(
                        CASE 
    ...
    --          )
    --    ORDER BY
    --        CASE WHEN C1.Address = H.PayToCode THEN 0 ELSE 1 END,
    --        C1.LineNum
    --) ExoCalc
	CROSS APPLY (
    SELECT
    ...
    OUTER APPLY (
        SELECT
            CASE
                ...
                WHEN COALESCE(T1.VatPrcnt,0) > 0
                THEN CAST(T1.VatPrcnt AS DECIMAL(18,5))
...
-- ✅ 4. CTE: Impuesto (base, monto y neto consistentes)
Impuesto AS (
    SELECT 
        IB.DocEntry,
        IB.LineNum, 
		...
		IB.DetalleServicio_BaseImponible   AS DetalleServicio_BaseImponible ,
		IB.DetalleServicio_ImpuestoMonto   AS DetalleServicio_ImpuestoMonto ,	
        ROUND(CASE 
    ...
    SELECT
        ...
        SUM(CASE
            WHEN Cabys.TipoCabys = 'M'
                 ...
                 AND T1.TaxOnly <> 'Y'
                 AND (
                     ISNULL(CASE WHEN T1.TaxCode = 'NS' THEN '07' ELSE '' END, '') = '07'
                 )
            THEN DS.DetalleServicio_MontoTotal
            ELSE 0
 ...
 ,ResumenFactura AS (
	SELECT	 
		...
		ROUND(	
			...
			,5) 
		  AS ResumenFactura_TotalVentaNeta,
...
TotalComprobanteFinal AS (
  ...
  FROM ResumenFactura RF
	...
	LEFT JOIN ResumenFactura RF ON 1 = 1 -- o CROSS JOIN ResumenFactura RF
	--LEFT JOIN InfoReferencia IR ON IR.DocNum = T0.DocNum
